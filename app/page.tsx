...
const MessagingInterface: React.FC = () => {
  const [chatSessions, setChatSessions] = useState<IChatSession[]>([]);
  const [filteredSessions, setFilteredSessions] = useState<IChatSession[]>([]);
  ...
  const loadChatSessions = useCallback(async (nextPage: number = 1) => {
    try {
      ...
      setChatSessions((prev) => [...prev, ...sortedChatSessions]);
      setFilteredSessions([...prev, ...sortedChatSessions]);
      setTotalPageCount(data.total_pages || null);
      setCurrentPage((prev) => prev + 1);
    } catch (error) {
      if (error instanceof Error) {
  ...
  // Search Filter
  useEffect(() => {
    const result = chatSessions.filter((session) =>
  ...
  const handleDateChange = (ranges: RangeKeyDict) => {
    ...
    const filteredByDate = chatSessions.filter((session) => {
      const sessionDate = new Date(session.messages[0]?.timestamp);
      return sessionDate >= startDate && sessionDate <= endDate;
    });

    setFilteredSessions(filteredByDate);
  };
